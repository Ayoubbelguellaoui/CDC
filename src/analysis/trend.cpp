...
std::string TrendAnalyzer::finding_key(const cdc::Finding& f) {
    ...
    for (const auto& value :
         {f.rule_id, f.source_reg_name, f.dest_reg_name, f.source_domain, f.dest_domain})
...
cdc::Finding TrendAnalyzer::deserialize_finding(const std::string& s) {
    ...
    while (std::getline(iss, line)) {
        ...
        if (key == "RULE64")
            f.rule_id = hex_decode(value);
        else if (key == "NAME64")
            f.rule_name = hex_decode(value);
        else if (key == "SEVERITY64")
            f.severity = hex_decode(value);
        else if (key == "SOURCE64")
            f.source_reg_name = hex_decode(value);
        else if (key == "DEST64")
            f.dest_reg_name = hex_decode(value);
        else if (key == "SRC_DOMAIN64")
            f.source_domain = hex_decode(value);
        else if (key == "DST_DOMAIN64")
            f.dest_domain = hex_decode(value);
        else if (key == "REASON64")
            f.reason = hex_decode(value);
        else if (key == "FILE64")
            f.source_loc.file = hex_decode(value);
        else if (key == "WAIVER_JUST64")
            f.waiver_justification = hex_decode(value);
        else if (key == "WAIVER_OWNER64")
            f.waiver_owner = hex_decode(value);
        else if (key == "RULE")
            f.rule_id = value;
        else if (key == "NAME")
            f.rule_name = value;
            ...
            f.source_reg_name = value;
        else if (key == "DEST")
            f.dest_reg_name = value;
        else if (key == "SRC_DOMAIN")
            f.source_domain = value;
        else if (key == "DST_DOMAIN")
            f.dest_domain = value;
        else if (key == "REASON")
            f.reason = value;
        else if (key == "FILE")
            f.source_loc.file = value;
        else if (key == "LINE") {
            try {
                f.source_loc.line = std::stoul(value);
            } catch (...) {
            }
        } else if (key == "WIDTH") {
            try {
                f.bus_width = std::stoul(value);
            } catch (...) {
            }
        } else if (key == "WAIVED")
            f.waived = (value == "1");
        else if (key == "WAIVER_JUST")
            f.waiver_justification = value;
        else if (key == "WAIVER_OWNER")
            f.waiver_owner = value;
    }
...
std::unordered_map<std::string, cdc::Finding> TrendAnalyzer::build_finding_map(
    ...
    std::unordered_map<std::string, size_t> occurrence_counters;
    for (const auto& f : findings) {
        std::string key = finding_key(f);
        size_t occurrence = occurrence_counters[key]++;
...
Baseline TrendAnalyzer::load_baseline(const std::string& filepath) {
    ...
    while (std::getline(file, line)) {
        ...
        if (key == "BASELINE64")
            baseline.name = hex_decode(value);
        else if (key == "VERSION64")
            baseline.version = hex_decode(value);
        else if (key == "BASELINE")
            baseline.name = value;
        else if (key == "TIMESTAMP") {
            try {
                baseline.timestamp = std::stol(value);
            } catch (...) {
            }
        } else if (key == "VERSION")
            ...
            try {
                baseline.total_errors = std::stoi(value);
            } catch (...) {
            }
        } else if (key == "WARNINGS") {
            try {
                baseline.total_warnings = std::stoi(value);
            } catch (...) {
            }
        } else if (key == "WAIVED") {
            try {
                baseline.total_waived = std::stoi(value);
            } catch (...) {
            }
