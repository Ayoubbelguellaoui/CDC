...
std::vector<Finding> Cdc006Analyzer::analyze(
    ...
    for (const auto& chain : chains) {
        ...
        for (size_t idx = 1; idx < chain.stage_ids.size(); ++idx) {
            ...
            f.reason = "Synchronizer chain " + chain_type + " at '" + first_stage->hier_name +
                       "' has combinational logic feeding stage " + std::to_string(idx + 1) +
                       " ('" + stage->hier_name +
                       "'). "
            ...
            f.safety_provenance = "Combinational logic detected between synchronizer stages";
        ...
        for (uint64_t pred_id : graph.predecessors(first_stage_id)) {
            ...
            for (uint64_t comb_pred : graph.predecessors(pred_id)) {
                ...
                if (cp && cp->kind == ir::NodeKind::Register &&
                    ...
                    f.safety_status = SafetyStatus::VerifiedUnsafe;
                    f.safety_provenance = "Combinational logic before first synchronizer stage";
