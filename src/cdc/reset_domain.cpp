...
ResetDomainResult ResetDomainAnalyzer::extract_reset_domains(const ir::Graph& graph) {
    ...
    for (const auto& node : graph.nodes()) {
        ...
        if (node.reset_signal.empty()) continue;
        
        std::string domain_key = node.reset_signal + "_" + 
                                std::to_string(static_cast<int>(node.reset_pol));
...
bool ResetDomainAnalyzer::is_async_reset_crossing(
    const ir::Node& src,
    const ir::Node& dst,
    ...
    if (src_reset_domain.name == dst_reset_domain.name &&
        src_reset_domain.polarity == dst_reset_domain.polarity) return false;
    
    // Async reset crossings are higher severity: the reset signal itself
    // crosses clock domains without synchronization
    return src.is_async_reset || dst.is_async_reset;
}

std::vector<Finding> ResetDomainAnalyzer::check_reset_crossings(
    const ir::Graph& graph,
    const std::vector<ResetDomain>&,
    ...
    for (const auto& src : graph.nodes()) {
        ...
        if (src.reset_signal.empty()) continue;
        
        auto src_it = reset_result.register_to_domain.find(src.id);
        if (src_it == reset_result.register_to_domain.end()) continue;
        if (src_it->second >= domains.size()) continue;
        ...
        for (uint64_t dst_id : graph.register_successors(src.id)) {
            ...
            if (dst->reset_signal.empty()) continue;
            
            auto dst_it = reset_result.register_to_domain.find(dst_id);
            if (dst_it == reset_result.register_to_domain.end()) continue;
            if (dst_it->second >= domains.size()) continue;
            const ResetDomain* dst_reset_domain = &domains[dst_it->second];
            
            if (src_reset_domain->name == dst_reset_domain->name &&
                src_reset_domain->polarity == dst_reset_domain->polarity) continue;
            
            auto clock_it = register_to_clock_domain.find(src.id);
            if (clock_it == register_to_clock_domain.end()) continue;
            if (clock_it->second >= clock_domains.size()) continue;
            ...
            if (src_clock->id == dst_clock->id) continue;
            ...
            if (async_crossing) {
                ...
                f.reason = "Async reset domain crossing: register '" + src.hier_name +
                          "' (async reset '" + src.reset_signal + "') feeds '" +
                          dst->hier_name + "' (reset '" + dst->reset_signal +
