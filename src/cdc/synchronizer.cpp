...
SyncPattern SynchronizerMatcher::find_pattern_for_dest(uint64_t dest_reg_id, const ir::Graph& graph,
    ...
    for (uint64_t s1 : graph.register_successors(dest_reg_id, false)) {
        ...
        // A sync stage should have exactly one same-domain register predecessor.
        // If it has more, it's a reconvergent consumer (e.g., a register fed by
        // two independent paths), not a chain stage.
        size_t same_domain_preds = 0;
        for (uint64_t pred : graph.register_predecessors(s1, false)) {
            const ir::Node* pn = graph.find_node(pred);
            if (pn && pn->kind == ir::NodeKind::Register && pn->clock_domain == n1->clock_domain) {
                ++same_domain_preds;
            }
        }
        if (same_domain_preds > 1)
            continue;
    ...
    for (uint64_t s2 : graph.register_successors(s1_id, false)) {
        ...
        // A sync stage should have exactly one same-domain register predecessor.
        // If it has more, it's a reconvergent consumer (e.g., a register fed by
        // two independent sync chains), not a chain stage.
        size_t same_domain_preds = 0;
        for (uint64_t pred : graph.register_predecessors(s2, false)) {
            const ir::Node* pn = graph.find_node(pred);
            if (pn && pn->kind == ir::NodeKind::Register && pn->clock_domain == n2->clock_domain) {
                ++same_domain_preds;
            }
        }
        if (same_domain_preds > 1)
    ...
    for (uint64_t s3 : graph.register_successors(s2_id, false)) {
        ...
        size_t same_domain_preds = 0;
        for (uint64_t pred : graph.register_predecessors(s3, false)) {
            const ir::Node* pn = graph.find_node(pred);
            if (pn && pn->kind == ir::NodeKind::Register && pn->clock_domain == n3->clock_domain) {
                ++same_domain_preds;
            }
        }
        if (same_domain_preds > 1)
