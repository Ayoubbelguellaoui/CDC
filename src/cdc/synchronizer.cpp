...
SyncPattern SynchronizerMatcher::find_pattern_for_dest(
    ...
    if (!has_cross_domain_register_pred) return SyncPattern::None;
    ...
    if (strict) {
        for (uint64_t pred_id : graph.register_predecessors(dest_reg_id, false)) {
            if (pred_id == dest_reg_id) continue;
            const ir::Node* pred = graph.find_node(pred_id);
            if (pred && pred->kind == ir::NodeKind::Register &&
                pred->clock_domain == dest->clock_domain) {
    ...
    for (uint64_t s1 : graph.register_successors(dest_reg_id, false)) {
        ...
        if (strict) {
            ...
            for (uint64_t pred : graph.register_predecessors(s1, false)) {
                ...
                if (!pn || pn->kind != ir::NodeKind::Register) continue;
                if (pred == dest_reg_id) continue;
                if (pred == s1) continue;
            ...
            if (has_unexpected_pred) continue;
        }
    ...
    if (!s3_id) return SyncPattern::TwoFF;  // s2 exists but no s3 — confirmed 2FF chain

    return SyncPattern::ThreeFF;  // s1, s2, and s3 all confirmed — 3FF chain
}
    ...
    for (const auto& node : graph.nodes()) {
        ...
        for (uint64_t pred_id : graph.register_predecessors(node.id, false)) {
            ...
            for (const auto& c : chains) {
                if (c.source_reg_id == pred_id &&
                    c.clock_domain == node.clock_domain) { exists = true; break; }
            }
            if (exists) continue;
