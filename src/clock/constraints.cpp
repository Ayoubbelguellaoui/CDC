...
bool pattern_matches(const std::string& pattern, const std::string& value) {
    ...
    if (iequals(pattern, value)) return true;

    bool has_wildcard = pattern.find('*') != std::string::npos ||
                        pattern.find('?') != std::string::npos;

    if (has_wildcard) {
        if (wildcard_match(pattern, value)) return true;
        ...
        for (int pass = 0; pass < 2; ++pass) {
            ...
            while (pos <= value.size()) {
                size_t next = (pass == 0)
                    ? value.find('.', pos)
                    : value.find_first_of("._", pos);
                std::string segment = value.substr(
                    pos, next == std::string::npos ? std::string::npos : next - pos);
                if (wildcard_match(pattern, segment)) return true;
...
static bool selector_matches(const std::string& pattern,
...
bool ClockConstraints::is_false_path(const PathMatchContext& ctx) const {
    for (const auto& fp : false_paths) {
        ...
        bool has_any_selector = from_clock_set || to_clock_set ||
                                from_reg_set || to_reg_set ||
                                from_cell_set || to_cell_set ||
                                from_pin_set || to_pin_set || through_set;

        if (!has_any_selector) continue;

        if (from_clock_set &&
            !selector_matches(fp.from_clock, ctx.source_clock))
            continue;
        if (to_clock_set &&
            !selector_matches(fp.to_clock, ctx.destination_clock))
            continue;
        if (from_reg_set &&
            !selector_matches(fp.from_reg, ctx.source_register))
            continue;
        if (to_reg_set &&
            !selector_matches(fp.to_reg, ctx.destination_register))
            continue;
        if (from_cell_set &&
            !selector_matches(fp.from_cell, ctx.source_cell))
            continue;
        if (to_cell_set &&
            !selector_matches(fp.to_cell, ctx.destination_cell))
            continue;
        if (from_pin_set &&
            !selector_matches(fp.from_pin, ctx.source_pin))
            continue;
        if (to_pin_set &&
            !selector_matches(fp.to_pin, ctx.destination_pin))
            continue;
        if (through_set &&
            !any_through_matches(fp.through, ctx.path_nodes))
            continue;
...
bool ClockConstraints::is_asynchronous(const std::string& clk1, const std::string& clk2) const {
    for (const auto& g1 : clock_groups) {
        if (!g1.asynchronous && !g1.exclusive) continue;
        for (const auto& g2 : clock_groups) {
            if (&g1 == &g2) continue;
            if (g1.set_id != g2.set_id) continue;

            bool clk1_in_g1 = false, clk2_in_g2 = false;
            for (const auto& clk : g1.clocks) {
                if (pattern_matches(clk, clk1)) clk1_in_g1 = true;
            }
            for (const auto& clk : g2.clocks) {
                if (pattern_matches(clk, clk2)) clk2_in_g2 = true;
            }
            if (clk1_in_g1 && clk2_in_g2) return true;

            bool clk2_in_g1 = false, clk1_in_g2 = false;
            for (const auto& clk : g1.clocks) {
                if (pattern_matches(clk, clk2)) clk2_in_g1 = true;
            }
            for (const auto& clk : g2.clocks) {
                if (pattern_matches(clk, clk1)) clk1_in_g2 = true;
            }
            if (clk2_in_g1 && clk1_in_g2) return true;
        }
    }
    return false;
}
...
std::optional<ClockDefinition> ClockConstraints::get_clock(const std::string& name) const {
    ...
    if (it != clock_map.end()) return it->second;
...
std::vector<std::string> SdcReader::extract_bracket_args(const std::string& token) {
    ...
    std::string rest;
    while (pos < inner.size()) rest += inner[pos++];
    ...
    if (cmd == "get_clocks" || cmd == "get_cells" || cmd == "get_pins") {
        ...
        if (!arg.empty()) args.push_back(strip_quotes(arg));
    }
        ...
        if (tok == "-name" && i + 1 < tokens.size()) {
            ...
            try {
                ...
                constraints.warnings.push_back(
                    "Invalid period value in create_clock: '" + tokens[i] + "'");
            } catch (const std::out_of_range&) {
                constraints.warnings.push_back(
                    "Period value out of range in create_clock: '" + tokens[i] + "'");
            }
        } else if (tok.find('/') != std::string::npos || 
                   (tok[0] != '-' && tok.find_first_of("0123456789") == std::string::npos)) {
            if (clk.name.empty()) {
                std::string port = tok;
...
void SdcReader::parse_create_generated_clock(const std::string& line, ClockConstraints& constraints) {
    auto tokens = tokenize(line);
    ClockDefinition clk;
...
void SdcReader::parse_set_false_path(const std::string& line, ClockConstraints& constraints) {
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
        ...
        if (tok == "-from" && i + 1 < tokens.size()) {
            ...
            if (!args.empty()) {
                for (const auto& a : args) fp.through.push_back(a);
            } else {
                auto strip = [](std::string s) -> std::string {
                    if (s.size() >= 2 && s.front() == '"' && s.back() == '"')
                        return s.substr(1, s.size() - 2);
...
void SdcReader::parse_set_multicycle_path(const std::string& line, ClockConstraints& constraints) {
    ...
    if (!from_is_clock && !to_is_clock) return;
    
    // Create entries for all combinations of from × to args.
    if (from_args.empty()) from_args = {""};
    if (to_args.empty()) to_args = {""};
...
void SdcReader::parse_set_clock_groups(const std::string& line, ClockConstraints& constraints) {
    ...
    for (const auto& tok : tokens) {
        if (tok == "-asynchronous") asynchronous = true;
        else if (tok == "-exclusive") exclusive = true;
    }
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
        ...
        } else if (tok[0] != '-' && tok.find("get_clocks") != std::string::npos) {
...
ClockConstraints SdcReader::read_sdc(const std::string& path) {
    ...
    if (!file.is_open()) return ClockConstraints{};
...
ClockConstraints SdcReader::parse_sdc_content(const std::string& content) {
    ...
    while (std::getline(iss, raw_line)) {
        ...
        while (!accumulated.empty() && std::isspace(accumulated.front())) {
...
void ConstraintsParser::parse_clocks_section(const std::string& content, ClockConstraints& constraints) {
    std::istringstream iss(content);
    std::string line;
        ...
        if (current_clock) {
            ...
            if (colon != std::string::npos) {
                ...
                if (key == "frequency" || key == "frequency_mhz") {
                    try { auto v = std::stod(value); if (std::isfinite(v) && v >= 0) current_clock->frequency_mhz = v; } catch (...) {}
                } else if (key == "period" || key == "period_ns") {
                    try { auto v = std::stod(value); if (std::isfinite(v) && v > 0) current_clock->period_ns = v; } catch (...) {}
                } else if (key == "source") {
                    ...
                    try { auto v = std::stod(value); if (std::isfinite(v) && v > 0) current_clock->divider_ratio = v; } catch (...) {}
                } else if (key == "multiplier" || key == "multiplier_ratio") {
                    try { auto v = std::stod(value); if (std::isfinite(v) && v > 0) current_clock->multiplier_ratio = v; } catch (...) {}
                }
            }
        }
    }
...
void ConstraintsParser::parse_false_paths_section(const std::string& content, ClockConstraints& constraints) {
    std::istringstream iss(content);
    std::string line;
    ...
    while (std::getline(iss, line)) {
        ...
        while (std::getline(fiss, token, ',')) {
            ...
            if (key == "from_clock" || key == "from") fp.from_clock = value;
            else if (key == "to_clock" || key == "to") fp.to_clock = value;
            else if (key == "from_reg") fp.from_reg = value;
            else if (key == "to_reg") fp.to_reg = value;
            else if (key == "source") fp.from_reg = value;
            else if (key == "dest") fp.to_reg = value;
            else if (key == "through") fp.through.push_back(value);
            else if (key == "reason") fp.reason = value;
        }
...
void ConstraintsParser::parse_multi_cycle_section(const std::string& content, ClockConstraints& constraints) {
    std::istringstream iss(content);
    std::string line;
    ...
    while (std::getline(iss, line)) {
        ...
        while (std::getline(fiss, token, ',')) {
            ...
            if (key == "from_clock" || key == "from") mcp.from_clock = value;
            else if (key == "to_clock" || key == "to") mcp.to_clock = value;
            else if (key == "cycles") {
                try { mcp.cycles = std::stoi(value); } catch (...) {}
            }
        }
...
void ConstraintsParser::parse_groups_section(const std::string& content, ClockConstraints& constraints) {
    std::istringstream iss(content);
    std::string line;
...
ClockConstraints ConstraintsParser::parse_yaml(const std::string& content) {
    ...
    while (std::getline(iss, line)) {
        ...
        if (lower == "clocks:" || lower == "false_paths:" || 
            lower == "multi_cycle_paths:" || lower == "clock_groups:") {
    ...
    if (!clocks_section.empty()) parse_clocks_section(clocks_section, constraints);
    if (!false_paths_section.empty()) parse_false_paths_section(false_paths_section, constraints);
    if (!multi_cycle_section.empty()) parse_multi_cycle_section(multi_cycle_section, constraints);
    if (!groups_section.empty()) parse_groups_section(groups_section, constraints);
...
ClockConstraints ConstraintsParser::parse_file(const std::string& path, std::string* error) {
    std::ifstream file(path);
    if (!file.is_open()) {
        if (error) *error = "Could not open constraints file: " + path;
    ...
    if (!trim(content).empty() && constraints.clocks.empty() &&
        constraints.false_paths.empty() && constraints.multi_cycle_paths.empty() &&
        constraints.clock_groups.empty()) {
        if (error) *error = "No recognized constraints found in: " + path;
        return ClockConstraints{};
    }

    for (const auto& clock : constraints.clocks) {
        if (clock.name.empty() ||
            !std::isfinite(clock.period_ns) || !std::isfinite(clock.frequency_mhz) ||
            clock.period_ns < 0.0 || clock.frequency_mhz < 0.0) {
            if (error) *error = "Invalid clock definition in: " + path;
            return ClockConstraints{};
        }
    }
    for (const auto& path_info : constraints.multi_cycle_paths) {
        if (path_info.cycles < 1 ||
            (path_info.from_clock.empty() && path_info.to_clock.empty())) {
            if (error) *error = "Invalid multicycle path in: " + path;
            return ClockConstraints{};
        }
    }
