...
static bool is_gray_xor_structure(const Expression& expr) {
    if (expr.kind != ExpressionKind::BinaryOp) return false;
    auto& binary = static_cast<const BinaryExpression&>(expr);
    if (binary.op != BinaryOperator::BinaryXor) return false;

    auto is_shift_of = [](const Expression& shifted, const Expression& base) -> bool {
        if (shifted.kind != ExpressionKind::BinaryOp) return false;
...
static void apply_pattern_name_tags(ir::Node& node) {
    ...
    std::string base = (last_dot == std::string::npos)
                           ? node.hier_name
                           : node.hier_name.substr(last_dot + 1);
    std::string l = base;
    ...
    bool fifo_ptr_named = l.find("wr_ptr") != std::string::npos ||
                          l.find("write_ptr") != std::string::npos ||
                          l.find("wptr") != std::string::npos ||
                          l.find("rd_ptr") != std::string::npos ||
                          l.find("read_ptr") != std::string::npos ||
                          l.find("rptr") != std::string::npos;
    if (fifo_ptr_named) {
        node.is_async_fifo_ptr = true;
    }
}

static std::optional<std::pair<std::string, uint32_t>> extract_lvalue(
    const Expression& expr, const std::string& prefix) {
...
static void collect_named_values(const Expression& expr,
    ...
    if (expr.kind == ExpressionKind::NamedValue) {
        ...
        if (!call.isSystemCall()) {
            for (auto* arg : call.arguments()) {
                if (arg) collect_named_values(*arg, prefix, out, formal_map);
            }
        }
    }
}
...
static void collect_assigns(const Statement& stmt,
    ...
    case StatementKind::Block: {
        ...
        collect_assigns(blk.body, prefix, out, formal_map);
        break;
    }
    case StatementKind::List: {
        auto& list = static_cast<const StatementList&>(stmt);
        for (auto* s : list.list)
            collect_assigns(*s, prefix, out, formal_map);
        break;
    }
    case StatementKind::Timed: {
        auto& timed = static_cast<const TimedStatement&>(stmt);
        collect_assigns(timed.stmt, prefix, out, formal_map);
        break;
    }
        ...
        if (es.expr.kind == ExpressionKind::Assignment) {
            ...
            if (lhs) {
                ...
                if (assign.right().kind == ExpressionKind::BinaryOp) {
                    ...
                    if (binary.op == BinaryOperator::BinaryAnd) logic_type = ir::LogicType::And;
                    else if (binary.op == BinaryOperator::BinaryOr) logic_type = ir::LogicType::Or;
                    else if (binary.op == BinaryOperator::BinaryXor) logic_type = ir::LogicType::Xor;
                ...
                if (rhs_ops.empty()) {
                    ...
                    // The register node is already created by add_register above;
                    // no edge needed for a const-init.
                } else if (rhs_ops.size() == 1 && !is_multi_operand) {
                    out.push_back({resolved_lhs, rhs_ops[0].first,
                                   l_width, rhs_ops[0].second, is_gray_transform,
                                   logic_type, false});
                } else if (rhs_ops.size() >= 1) {
                    for (auto& [r, r_width] : rhs_ops) {
                        out.push_back({resolved_lhs, r, l_width, r_width,
                                       is_gray_transform, logic_type, true});
                    }
                }
            }
    ...
    case StatementKind::Conditional: {
        ...
        collect_assigns(ifst.ifTrue, prefix, out, formal_map);
        if (ifst.ifFalse)
            collect_assigns(*ifst.ifFalse, prefix, out, formal_map);
        break;
    }
    case StatementKind::ForLoop: {
        auto& fl = static_cast<const ForLoopStatement&>(stmt);
        collect_assigns(fl.body, prefix, out, formal_map);
        break;
    }
    case StatementKind::WhileLoop: {
        auto& wl = static_cast<const WhileLoopStatement&>(stmt);
        collect_assigns(wl.body, prefix, out, formal_map);
        break;
    }
    case StatementKind::DoWhileLoop: {
        auto& dwl = static_cast<const DoWhileLoopStatement&>(stmt);
        collect_assigns(dwl.body, prefix, out, formal_map);
        break;
    }
    case StatementKind::RepeatLoop: {
        auto& rl = static_cast<const RepeatLoopStatement&>(stmt);
        collect_assigns(rl.body, prefix, out, formal_map);
        break;
    }
...
static bool is_reset_name(const std::string& name) {
    ...
    if (bracket != std::string::npos) lower = lower.substr(0, bracket);
    return lower.find("rst")    != std::string::npos ||
           lower.find("reset")  != std::string::npos ||
           lower.find("arst")   != std::string::npos ||
           lower.find("clr")    != std::string::npos ||
           lower.find("clear")  != std::string::npos ||
           lower.find("preset") != std::string::npos;
...
std::string SlangAdapter::event_signal_name(const TimingControl& event) const {
    if (event.kind != TimingControlKind::SignalEvent) return "";
...
ClockResetInfo SlangAdapter::extract_clock_reset(
    ...
    if (timing.kind == TimingControlKind::SignalEvent) {
        ...
        for (auto& ev : el.events) {
            if (!ev || ev->kind != TimingControlKind::SignalEvent) continue;
            auto& se = static_cast<const SignalEventControl&>(*ev);
            if (se.edge != EdgeKind::NegEdge && se.edge != EdgeKind::PosEdge) continue;
...
FrontendResult SlangAdapter::elaborate(const std::vector<std::string>& files,
    ...
    for (auto& diag : diags) {
        if (diag.isError()) has_errors = true;
    }
    if (has_errors) {
    ...
    if (result.graph.truncated()) {
        result.errors.push_back(
            "IR graph exceeded node or edge safety limit; analysis incomplete");
        return result;
    }
...
void SlangAdapter::walk_instance(const InstanceSymbol& inst,
    ...
    walk_scope(inst.body, graph, p, qualify_clocks);
}

void SlangAdapter::walk_scope(const slang::ast::Scope& scope,
                              ir::Graph& graph,
                              const std::string& p,
                              bool qualify_clocks) {
    ...
    for (auto& member : scope.members()) {
        ...
        case SymbolKind::Instance: {
            ...
            walk_scope(child.body, graph, p + "." + sv_to_string(child.name), true);
            ...
            for (const auto* connection : child.getPortConnections()) {
                ...
                if (parent_values.empty()) continue;

                const std::string child_port_name = child_prefix + "." +
                    sv_to_string(child_port.name);
                const ir::Node* child_node =
                    graph.find_node_by_name(child_port_name);
                if (!child_node) continue;

                for (const auto& [parent_name, parent_width] : parent_values) {
                    const ir::Node* parent_node =
                        graph.find_node_by_name(parent_name);
        ...
        case SymbolKind::GenerateBlockArray: {
            ...
            for (const auto* entry : array.entries) {
                if (entry && !entry->isUninstantiated) {
                    walk_scope(*entry, graph,
                               array_prefix + "." + sv_to_string(entry->name));
                }
            }
            break;
        }
            ...
            if (proc.procedureKind == ProceduralBlockKind::AlwaysFF ||
                ...
                for (auto& a : assigns) {
                    ...
                    uint64_t dst_id = dst ? dst->id
                                          : (cr.clock == "comb"
                                             ? graph.add_net(a.lhs, a.lhs_width, ir_loc, p)
    ...
    for (auto& member : scope.members()) {
        if (member.kind != SymbolKind::ContinuousAssign) continue;
        auto& ca = static_cast<const ContinuousAssignSymbol&>(member);
        const Expression& assign_expr = ca.getAssignment();
        if (assign_expr.kind != ExpressionKind::Assignment) continue;
        ...
        if (asgn.right().kind == ExpressionKind::BinaryOp) {
            ...
            if (bin.op == BinaryOperator::BinaryAnd) logic_type = ir::LogicType::And;
            else if (bin.op == BinaryOperator::BinaryOr) logic_type = ir::LogicType::Or;
            else if (bin.op == BinaryOperator::BinaryXor) logic_type = ir::LogicType::Xor;
        ...
        if (!rhs_ops.empty()) {
            ...
            uint64_t lhs_id = existing_lhs ? existing_lhs->id
                                           : graph.add_net(lhs_name, lhs_width, {}, p);
            if (!existing_lhs) {
         ...
         if (asgn.right().kind == ExpressionKind::ConditionalOp) {
            ...
            for (auto& node : graph.nodes_mutable()) {
                if (node.kind == ir::NodeKind::Register &&
                    (node.clock_domain == wire_name ||
                     node.clock_domain == qualified_wire)) {
...
void SlangAdapter::resolve_clocks(ir::Graph& graph) {
    ...
    for (const auto& node : graph.nodes()) {
        if (node.kind != ir::NodeKind::Port) continue;
    ...
    for (auto& node : graph.nodes_mutable()) {
        ...
        if (node.root_clock.empty() || !is_top(node.root_clock)) {
            std::string source = trace_to_top_level_source(
                node.clock_domain, graph);
...
bool SlangAdapter::is_top_level_input(const std::string& hier_name,
                                      const ir::Graph& graph) {
    for (const auto& node : graph.nodes()) {
        if (node.kind != ir::NodeKind::Port) continue;
        if (node.hier_name == hier_name) {
            size_t dots = 0;
            for (char c : hier_name) if (c == '.') dots++;
            return dots == 1;
        }
    }
    for (const auto& node : graph.nodes()) {
        ...
        if (dot != std::string::npos) {
            std::string sn = node.hier_name.substr(dot + 1);
            if (sn == hier_name) {
                size_t dots = 0;
                for (char c : node.hier_name) if (c == '.') dots++;
                return dots == 1;
            }
        }
    }
...
std::string SlangAdapter::trace_to_top_level_source(
    ...
    if (!start) {
        for (const auto& node : graph.nodes()) {
            if (node.hier_name == clock_name) { start = &node; break; }
            size_t dot = node.hier_name.rfind('.');
            if (dot != std::string::npos &&
                node.hier_name.substr(dot + 1) == clock_name) {
                start = &node;
                break;
            }
        }
    }
    if (!start) return clock_name;

    if (start->kind == ir::NodeKind::Port &&
        is_top_level_input(start->hier_name, graph)) {
        size_t dot = start->hier_name.rfind('.');
        return (dot != std::string::npos)
            ? start->hier_name.substr(dot + 1) : start->hier_name;
    }
    ...
    for (int depth = 0; depth < 20; ++depth) {
        ...
        if (cn->kind == ir::NodeKind::Port &&
            is_top_level_input(cn->hier_name, graph)) {
            size_t dot = cn->hier_name.rfind('.');
            return (dot != std::string::npos)
                ? cn->hier_name.substr(dot + 1) : cn->hier_name;
        }
        ...
        for (uint64_t pred_id : graph.predecessors(cn->id)) {
            ...
            if (pred->kind == ir::NodeKind::Port) {
                if (is_top_level_input(pred->hier_name, graph)) {
                    size_t dot = pred->hier_name.rfind('.');
                    return (dot != std::string::npos)
                        ? pred->hier_name.substr(dot + 1) : pred->hier_name;
                }
                continue;
            }

            if (pred->kind == ir::NodeKind::Net ||
                pred->kind == ir::NodeKind::Combinational) {
