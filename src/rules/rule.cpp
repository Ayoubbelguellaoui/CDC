...
RuleEngine::RuleEngine() {
    register_rule({"CDC001", "unsynchronized_crossing",
                   "Register drives register across clock domains without synchronization", "error",
                   true, "crossing", "1.0.0"});
    register_rule({"CDC002", "multi_bit_crossing",
                   "Multi-bit bus crosses clock domains without gray-code or handshake", "error",
                   true, "crossing", "1.0.0"});
    register_rule({"CDC003", "reconvergence_hazard",
                   "Multiple paths from same source reconverge in destination domain", "warning",
                   true, "reconvergence", "1.0.0"});
    register_rule({"CDC004", "gated_clock_crossing",
                   "Register clocked by gated clock crosses to another domain", "warning", true,
                   "crossing", "1.0.0"});
    register_rule({"CDC005", "muxed_clock_no_reset",
                   "Register clocked by muxed clock without reset", "warning", true, "crossing",
                   "1.0.0"});
    ...
    register_rule({"CDC009", "reset_domain_crossing",
                   "Register crosses between different asynchronous reset domains", "warning", true,
                   "reset", "1.0.0"});
