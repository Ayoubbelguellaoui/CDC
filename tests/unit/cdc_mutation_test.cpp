...
TEST_F(CdcMutationTest, InsertCombBetweenStagesTriggersCdc006) {
    ...
    uint64_t comb = graph.add_combinational("mod.and_gate", LogicType::And, {extra, meta}, 1,
                                            {"mod.sv", 11, 1});
...
TEST_F(CdcMutationTest, NamingDoesNotBypassCdc002) {
    ...
    EXPECT_TRUE(found_cdc002)
        << "Register named 'gray_data' without structure should still trigger CDC002";
}
...
TEST_F(CdcMutationTest, ResetDomainCrossingTriggersCdc009) {
    ...
    for (const auto& f : rfindings) {
        if (f.rule_id == "CDC009") {
            ...
            EXPECT_FALSE(f.safety_provenance.empty()) << "CDC009 should have safety_provenance";
        }
    }
}
...
TEST_F(CdcMutationTest, PathTruncationTriggersCdc010) {
    ...
    for (int level = 0; level < kTreeLevels; ++level) {
        ...
        for (size_t i = 0; i < prev_level.size(); ++i) {
            ...
            std::string c2_name =
                "mod.c_" + std::to_string(level) + "_" + std::to_string(i * 2 + 1);
            uint64_t c1 = graph.add_combinational(
                c1_name, LogicType::None, {prev_level[i]}, 1,
                {"mod.sv", 100 + level * 1000 + static_cast<uint32_t>(i * 2), 1});
            uint64_t c2 = graph.add_combinational(
                c2_name, LogicType::None, {prev_level[i]}, 1,
                {"mod.sv", 100 + level * 1000 + static_cast<uint32_t>(i * 2 + 1), 1});
            ...
            if (level == kTreeLevels - 1) {
                ...
                uint64_t leaf1 = graph.add_register(
                    r1_name, "clk_b", 1, {"mod.sv", 200 + static_cast<uint32_t>(i * 2), 1});
                uint64_t leaf2 = graph.add_register(
                    r2_name, "clk_b", 1, {"mod.sv", 200 + static_cast<uint32_t>(i * 2 + 1), 1});
